...
ProtocolMessage ProtocolMessage::deserialize(const char *const buffer,
                                             const size_t numBytes)
{
  ...
  case CommandType::ALARM:
    ...
    return ProtocolMessage(
        AlarmCommandBuilder::buildAlarmCommand(buffer + 1, numBytes - 1));
...
AlarmCommandBuilder::AlarmCommandBuilder()
    : level(AlarmCommand::Level::Level1), idLength(0), idBuffer({}),
      designatorLength(0), designatorBuffer({}), messageLength(0),
      messageBuffer({}) {}

size_t AlarmCommandBuilder::deserializeLevelBytes(const char *const buffer,
                                                  const size_t numBytes)
{
  if (numBytes == 0)
  {
...
size_t AlarmCommandBuilder::deserializeIdBytes(const char *const buffer,
                                               const size_t numBytes)
{
...
size_t
AlarmCommandBuilder::deserializeTypeDesignatorBytes(const char *const buffer,
                                                    const size_t numBytes)
{
  if ((numBytes == 0) ||
      buffer[0] != AlarmCommandBuilder::DESIGNATOR_START_CHARACTER)
  {
    return 0;
  }
...
size_t AlarmCommandBuilder::deserializeMessageBytes(const char *const buffer,
                                                    const size_t numBytes)
{
...
AlarmCommand AlarmCommandBuilder::buildAlarmCommand(const char *const buffer,
                                                    const size_t numBytes)
{
  ...
  const auto message =
      AlarmMessage(builder.messageLength, std::move(builder.messageBuffer));

  return AlarmCommand(builder.level, std::move(message), std::move(messageId),
